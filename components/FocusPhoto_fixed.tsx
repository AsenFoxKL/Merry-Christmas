...
const FocusPhoto: React.FC<FocusPhotoProps> = ({ 
  ...
  useFrame((state) => {
    ...
    // On the very first frame, run the animation step multiple times to ensure visibility
    if (isFirstFrameRef.current) {
      isFirstFrameRef.current = false;
      // Run 3 animation steps immediately to ensure the animation is visible on first frame
      for (let i = 0; i < 3; i++) {
